...
from networks.qmix import QMixer
import torch as th
from torch.optim import RMSprop
from llm.base import MetricsHistory

class QLearner:
    def __init__(self, mac, logger, args, metrics_history=None):
        self.args = args
        self.mac = mac
        self.logger = logger
    ...
    def load_models(self, path):
        ...
        self.target_mac.agent.load_state_dict(
            th.load("{}/target_agent.th".format(path), map_location=lambda storage, loc: storage, weights_only=True))
        if self.mixer is not None:
            self.mixer.load_state_dict(
                th.load("{}/mixer.th".format(path), map_location=lambda storage, loc: storage, weights_only=True))
            self.target_mixer.load_state_dict(
                th.load("{}/target_mixer.th".format(path), map_location=lambda storage, loc: storage, weights_only=True))
        self.optimiser.load_state_dict(
            th.load("{}/opt.th".format(path), map_location=lambda storage, loc: storage, weights_only=True))
